...
export async function PATCH(request: Request) {
  try {
    ...
    await logActivity({actorUserId:auth.session!.id,actorUsername:auth.session!.username,actorRole:auth.session!.role,action:"UPDATE",module:"PERMISSION",targetType:"UserPermission",targetId:userId,description:`Mengubah permission ${permissionCode} untuk akun ${user.username} menjadi ${body.allowed ? "diizinkan" : "ditolak"}.`,metadata:{permissionCode,allowed:body.allowed},rTUnitId:user.rTUnitId,request});
    return NextResponse.json({
      success: true,
      message:
        body.allowed === true
