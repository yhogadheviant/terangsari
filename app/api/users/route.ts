...
export async function POST(request: Request) {
  try {
    ...
    await logActivity({actorUserId:auth.session!.id,actorUsername:auth.session!.username,actorRole:auth.session!.role,action:"CREATE",module:"AKUN",targetType:"User",targetId:row.id,description:`Membuat akun ${row.username} dengan role ${row.role}.`,metadata:{after:{username:row.username,role:row.role,rTUnitId:row.rTUnitId}},rTUnitId:row.rTUnitId,request});
    return NextResponse.json(
      {
        success: true,
...
export async function PATCH(request: Request) {
  try {
    ...
    await logActivity({actorUserId:auth.session!.id,actorUsername:auth.session!.username,actorRole:auth.session!.role,action:"UPDATE",module:"AKUN",targetType:"User",targetId:row.id,description:`Mengubah akun ${row.username}.`,metadata:{before:{username:existing.username,role:existing.role,rTUnitId:existing.rTUnitId},changes:data,after:{username:row.username,role:row.role,rTUnitId:row.rTUnitId}},rTUnitId:row.rTUnitId,request});
    return NextResponse.json({
      success: true,
