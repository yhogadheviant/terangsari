...
export async function PATCH(
  ...
  try {
    ...
    if (actor) await logActivity({actorUserId:actor.id,actorUsername:actor.username,actorRole:actor.role,action:"UPDATE",module:"PENGATURAN",targetType:"AppSetting",targetId:setting.id,description:`Mengubah nama aplikasi menjadi "${setting.value}".`,metadata:{key:KEY,value:setting.value},request});
    return NextResponse.json({
      success: true,
      message:
